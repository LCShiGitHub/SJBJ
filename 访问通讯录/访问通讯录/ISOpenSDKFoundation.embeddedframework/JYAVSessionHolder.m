...
-(JYVivoVideoView*)videoView
{
    ...
    if (_videoView == nil)
    {
        [self setVideoView:[[JYVivoVideoView alloc] init]];
    }
...
-(void)takePicture:(JYPictureTaked)taked
{
    ...
    [[self stillImageOutput] captureStillImageAsynchronouslyFromConnection:[[self stillImageOutput] connectionWithMediaType:AVMediaTypeVideo] completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error)
    {
        ...
        if (imageDataSampleBuffer)
        {
            ...
            if (imageData)
            {
                self.IDPhotoImageData = imageData;
                
                UIImage *image = [UIImage imageWithData:imageData];
                
                image = [self imageOrientation:image rotation:UIImageOrientationLeft];//图片转向
                ...
                CVPixelBufferRef imageBuffer = [self pixelBufferFromCGImage:(sourceImageRef)];//生成灰度值
                ...
                SInputGrayBufInfo *bufInfo = [self createGrayBufInfo:imageBuffer];// 从buf生成灰度值信息的结构体//传imageBuffer进去了
                ...
                NSData *tmpImageData = UIImageJPEGRepresentation(tmpImage, 0.6);
                self.mode.idCardData = tmpImageData;
                //保存照片到本地
                [self savePhotoToLocalWithData:tmpImageData];
            }
            if (pxdata)
            {
                free(pxdata);  //释放
...
//帧数据判断并取身份证照片
-(BOOL)judgeAndObtainIdPhoto:(CMSampleBufferRef)sampleBuffer
{
    
    
    //取身份证
    if (self.mode.photoGraph == NO && self.mode.adopt == NO && cardIng == NO && focusing == YES &&self.mode.severalController == 0)//判断，如果在第0个控制器时候，并且没有正在获取，并且没有手动获取，才获取
    {
        ...
        self.IDPhotoImageData = UIImageJPEGRepresentation(image, 0.8);
        ...
        CVPixelBufferRef imageBuffer = [self pixelBufferFromCGImage:(sourceImageRef)];//调用的方法，内存泄漏
        ...
        int number = getFacePos(pGrayBufInfo, &n,  &m, &p , &q);
        ...
        if (number == 0 )
        {
            ...
            {
                ntmp++;
                if (CAN_GET_PHOTOS)//取第二次没有聚焦的照片
                {
                    [self getOnePhotoWithSampleBuffer:sampleBuffer];
                }
            }else if (IDCARD_RECT_2)//避免误差
            {
                ntmp++;
                
                if (CAN_GET_PHOTOS) //取第二次没有聚焦的照片
                {
                    [self getTwoPhotoWithSamlpeBuffer:sampleBuffer];
                }
            }else
            {
                //识别不了
...
//    BOOL isSuccess = [self judgeAndObtainIdPhoto:sampleBuffer]; //获取判断并身份证照片
//    if (isSuccess) {
//        return;
//    }
...
-(void)getOnePhotoWithSampleBuffer:(CMSampleBufferRef)sampleBuffer
{
    ...
    idCardData =UIImageJPEGRepresentation(upImage,0.8);
...
-(void)getTwoPhotoWithSamlpeBuffer:(CMSampleBufferRef)sampleBuffer
{
    ...
    idCardData =UIImageJPEGRepresentation(rightImage,0.8);
