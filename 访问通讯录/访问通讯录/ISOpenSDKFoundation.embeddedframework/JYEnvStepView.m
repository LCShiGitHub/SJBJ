...
-(void)layoutSubviews
{
    ...
    self.scanlineImageView.frame = CGRectMake(0, 0, size.width-140,((size.width-140)/60)*77+13);//扫描线宽度
    self.statusLabel.frame = CGRectMake(0, self.faceFrameImageView.frame.origin.y+self.faceFrameImageView.frame.size.height+20, size.width, 20);
...
-(void)next
{
    if (_next)
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"StartHtjc" object:self];
        _next();
    }
}
...
-(NSString*)stepEnter
{
    ...
    [[JYAVSessionHolder instance] debugGrayImage:^(id some)
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            self.debugImageView.image = some;
